    ...
    public class TasinmazService : ITasinmazService
    {
        ...
        public async Task<ServiceResponse<List<TasinmazDto>>> GetAllAsync(int logKullaniciId)
        {
            ...
            try
            {
                ...
                if (tasinmazlarAllList.Count == 0)
                {
                    response.Process = "Taşınmazlar";
                    response.Message = "Taşınmazlar veri tabanında bulunamadı.";
        ...
        public async Task<ServiceResponse<TasinmazDto>> DeleteTasinmazAsync(int logKullaniciId, int kullaniciId)
        {
            ...
            try
            {
                ...
                if (!await _tasinmazRepository.DeleteAsync(x=>x.Id == kullaniciId))
                {
                    ...
                    log.Durum = "Başarısız";
                    log.Islem = "Silme";
                    log.Aciklama = $"Açıklama: \"Taşınmaz veri tabanından silinirken bir hata oluştu.\"";
                    await _logService.AddLogAsync(log);
                    return response;
                }
                response.Process = "Taşınmazlar";
            ...
            catch (Exception e)
            {
                ...
                log.Durum = "Başarısız";
                log.Islem = "Silme";
                log.Aciklama = $"Açıklama: \"Taşınmaz servis katmanından silinirken bir hata oluştu.\" - Hata Mesajları: \"{JsonConvert.SerializeObject(response.ErrorMessages)}\"";
                await _logService.AddLogAsync(log);
            }
            return response;
        }
